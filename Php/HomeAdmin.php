...
$res = $con->connection->prepare("SELECT NOME FROM ADMIN WHERE MATRICULA = ?");
$res->execute(array($_SESSION["matriculaAdmin"]));
$result = $res->fetchAll();
$_SESSION["nome"] = $result[0]["NOME"];

$res = $con->connection->query("SELECT ID, NOME, MATRICULA FROM FUNCIONARIO");
$funcionario = $res->fetchAll();
            ...
            <div class="row px-0 mx-auto text-center w-100">
                <div class="col-md-12 w-100 mx-auto p-0">
                    <form class="btn-group w-100 mx-auto" method="post" action="">
                        ...
                        <button type="submit" name="funcionario" class="btn btn-success w-50">Lista de Funcionário
                        </button>
                        <a href="AdicionarEstudante.php" class="btn btn-success w-50">Adicionar Estudantes</a>
                        <a href="AdicionarFuncionario.php" class="btn btn-success w-50">Adicionar Funcionários</a>
                    </form>
                </div>
            </div>
        </div>
            ...
            <div class="row">
                <div class="col-md-12">
                    <table class="table">
                        ...
                        <tbody>
                        <?php foreach ($funcionario as $tableFuncionario): ?>
                            <?= "<tr>" ?>
                            <?= "<td>" . $tableFuncionario["ID"] . "</td>" ?>
                            <?= "<td>" . $tableFuncionario["NOME"] . "</td>" ?>
                            <?= "<td>" . $tableFuncionario["MATRICULA"] . "</td>" ?>
                            <?= "<td>
                                            <button onclick=window.location=\"Delete.php?deleteFuncionario={$tableFuncionario["ID"]}\">
                                                <i class='fa fa-pencil' aria-hidden='true'></i>
                                            </button>&emsp;    
                                            <button onclick=window.location=\"Delete.php?deleteFuncionario={$tableFuncionario["ID"]}\">
                                                <i class='fa fa-times' aria-hidden='true'></i>
